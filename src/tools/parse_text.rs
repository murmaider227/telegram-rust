...
pub async fn parse_text(text: &str) -> String {
    let re = Regex::new(r"([0-9.,]+)\s+([a-zA-Z]+)").unwrap();
    let text = text.to_uppercase();
    let result = re.captures_iter(&text);
    let mut coins = Vec::new();

    for cap in result {
        let amount = cap.get(1).unwrap().as_str().to_owned();
        let currency = cap.get(2).unwrap().as_str().to_owned();
        coins.push((amount, currency));
    }

    if !coins.is_empty() {
        parser_coins_mult(&coins).await.unwrap_or("".to_string())
    } else {
        "".to_string()
    }
}
